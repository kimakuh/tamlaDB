...
public class TourController {
    ...
    @RequestMapping("add")
    public String add(Tour tour) throws Exception {
    ...
    public String delete(@RequestParam("no") int no) throws Exception {
        ...
        int count = tourService.delete(no);
        if (count == 0) {
    ...
    public void list(
        ...
        map.put("list", tourService.list(pageNo, pageSize));
    }
    
    @RequestMapping("update")
    public String update(Tour tour) throws Exception {
        
        int count = tourService.update(tour);
        if (count == 0) {
...
//ver 39 - forward 적용
//ver 38 - redirect 적용
//ver 37 - TourAddController 클래스를 서블릿으로 변경
//         출력 결과를 HTML로 변경
//         자동 Refresh 태그 추가
//ver 31 - JDBC API가 적용된 DAO 사용
//ver 28 - 네트워크 버전으로 변경
//ver 26 - TourController에서 add() 메서드를 추출하여 클래스로 정의. 
